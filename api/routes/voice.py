...
def get_sample_status(user_id: UUID):
...
def analyze_voice(request: AnalyzeRequest):
...
def get_profile_by_id(profile_id: UUID):
...
def update_profile(profile_id: UUID, request: UpdateProfileRequest):
...
def clone_profile(profile_id: UUID, request: CloneProfileRequest):
...
def set_default_profile(profile_id: UUID, user_id: UUID):
