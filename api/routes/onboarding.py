...
def get_existing_strategy(user_id: UUID, platform_id: Optional[UUID] = None):
...
def start_onboarding(request: StartRequest):
...
def quick_onboarding(request: QuickModeRequest):
...
def send_deep_message(request: DeepModeMessageRequest):
...
def generate_from_deep(request: GeneratePlanRequest):
...
def approve_plan(request: ApprovePlanRequest):
