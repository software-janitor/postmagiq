...
def upgrade() -> None:
    ...
    op.create_index('ix_workflow_configs_slug', 'workflow_configs', ['slug'], unique=True)

    # Index for name lookups
    op.create_index('ix_workflow_configs_name', 'workflow_configs', ['name'])

    # Index for environment filtering
    op.create_index('ix_workflow_configs_environment', 'workflow_configs', ['environment'])

    # Index for enabled configs
    op.create_index('ix_workflow_configs_enabled', 'workflow_configs', ['enabled'])

    # Index for tier filtering
    op.create_index('ix_workflow_configs_tier_required', 'workflow_configs', ['tier_required'])
